...
def get_top_two(test_df):
    new_df = test_df.copy()   
    return df.loc[df['area'].isin(new_df['area'].nlargest(2))]
...
def plot_population(test_df):
    ...
    axes.bar(new_df['country'], new_df['population'])
...
def plot_area(test_df):
    new_df = test_df.copy()
    fig, axes = plt.subplots(ncols=1, nrows=1, figsize=(10,10))
    axes.pie(new_df['area'], labels=new_df['country'])
    fig.suptitle('Area of Countries')
    plt.show()
    return fig