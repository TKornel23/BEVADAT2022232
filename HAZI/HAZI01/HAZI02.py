...
def compare_two_array(array_1, array_2):
    return np.array_equal(array_1, array_2, True)
...
def get_array_shape(input):
    array = np.array(input)
    return f"sor: {array.shape[0]}, oszlop: {array.shape[1]}, melyseg: {array.shape[2]}"
# %%
...
def repalce_odd_numbers(inputs):
    ...
    return map(lambda x: -1 if x % 2 == 0 else x, array)
...
def replace_by_value(input_list, a):
    ...
    return map(lambda x: 1 if x >= a else -1, array)
